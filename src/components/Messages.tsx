...
const Messages: FC<MessagesProps> = ({initialMessages , sessionId , chatId ,sessionImg, chatPartner}) => {
    ...
    const formatTimestamp = (timestamp: number) => {
      ...
      useEffect(() => {
        ...
        const messageHandeler = (message: Message) => {
            setMessages((prev)=>[message, ...prev]);
        };
        pusherClient.bind(`incomming-message`, messageHandeler);
    
        return () => {
          pusherClient.unsubscribe(
            toPusherKey(`chat:${chatId}`)
          );
          pusherClient.unbind(`incomming-message`, messageHandeler);
        };
      },[chatId]);
  ...
  return <div id='messages' className='flex h-full flex-1 flex-col-reverse  gap-4 p-3 overflow-y-auto scrollbar-thumb-blue scrollbar-thumb-rounded scrollbar-track-blue-lighter scrollbar-w-2 scrolling-touch'>
    <div className="" ref={scrollDownRef}/>
